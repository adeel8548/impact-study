...
export async function updateFeeStatus(feeId: string, status: "paid" | "unpaid", paidDate?: string) {
  const supabase = await createClient()
  ...
  const { error } = await supabase.from("student_fees").update(updateData).eq("id", feeId)
...
export async function checkFeeExpiration() {
  ...
  if (expiredFees && expiredFees.length > 0) {
    ...
    await supabase.from("student_fees").update({ status: "unpaid" }).in("id", ids)
  }
...
export async function getStudentFeeStatus(feeId: string) {
  ...
  if (fee.status === "paid" && fee.paid_date) {
    ...
    if (isPaidExpired) {
      ...
      await supabase.from("student_fees").update({ status: "unpaid" }).eq("id", feeId)
