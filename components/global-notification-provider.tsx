...
export function GlobalNotificationProvider() {
  ...
  const playWebAudioSound = useCallback(() => {
    try {
      console.log(
        "Playing notification via Web Audio API, audioContext initialized:",
  ...
  useEffect(() => {
    ...
    const checkAuthAndSetup = async () => {
      ...
      if (!userId) {
        console.log(
          "User not found in localStorage, skipping notification setup",
        );
        return;
      }
      ...
      try {
        ...
        console.log(
          "GlobalNotificationProvider: Firebase ready, setting up listeners",
        );
        ...
        const unsubAdminConv = onSnapshot(adminConvQ, (snap) => {
          ...
          convIds.forEach((convId) => {
            ...
            convSubsRef[convId] = onSnapshot(msgsQ, (msgsSnap) => {
              console.log(
                `Messages snapshot for conversation ${convId}:`,
                msgsSnap.size,
                "docs",
              );
              msgsSnap.docChanges().forEach((change) => {
                if (change.type === "added") {
                  ...
                  console.log(
                    `Message added: ${msgId}, from: ${msgData.senderId}, timestamp: ${msgTimestamp}`,
                  );
                  ...
                  if (msgData.senderId !== userId && msgData.isRead === false) {
                    ...
                    const lastTimestamp =
                      lastMessageTimestampRef.current[convId] || 0;

                    console.log(
                      `Message age: ${messageAge}ms, userId: ${userId}, isRead: ${msgData.isRead}, document.hidden: ${document.hidden}`,
                    );
                    ...
                    if (shouldPlay) {
                      ...
                      if (audioContextRef.current) {
                        if (audioContextRef.current.state === "suspended") {
                          audioContextRef.current
                            ...
                            .then(() => {
                              console.log(
                                "Audio context resumed, playing sound",
                              );
                      ...
                      console.log(
                        `Message too old (${messageAge}ms) or already seen, skipping sound`,
                      );
                    }
                  }
                }
        ...
        const unsubTeacherConv = onSnapshot(teacherConvQ, (snap) => {
          ...
          convIds.forEach((convId) => {
            ...
            convSubsRef[convId] = onSnapshot(msgsQ, (msgsSnap) => {
              console.log(
                `Messages snapshot for teacher conversation ${convId}:`,
                msgsSnap.size,
                "docs",
              );
              msgsSnap.docChanges().forEach((change) => {
                if (change.type === "added") {
                  ...
                  console.log(
                    `Teacher message added: ${msgId}, from: ${msgData.senderId}, timestamp: ${msgTimestamp}`,
                  );
                  ...
                  if (
                    ...
                    (msgData.isRead === false || msgData.isRead === undefined)
                  ) {
                    const now = Date.now();
                    const messageAge = now - msgTimestamp;
                    const lastTimestamp =
                    ...
                    console.log(
                      `Teacher message age: ${messageAge}ms, userId: ${userId}, document.hidden: ${document.hidden}`,
                    );
                    ...
                    if (shouldPlay) {
                      ...
                      console.log(
                        "Triggering notification sound for teacher message...",
                      );
                      playedMessageIdsRef.current.add(msgId);
                      ...
                      if (audioContextRef.current) {
                        if (audioContextRef.current.state === "suspended") {
                          audioContextRef.current
                            ...
                            .then(() => {
                              console.log(
                                "Audio context resumed for teacher message, playing sound",
                              );
                              playNotificationSound();
                            })
                            .catch((err) => {
                      ...
                      console.log(
                        `Teacher message too old (${messageAge}ms), skipping sound`,
                      );
                    }
                  }
                }
