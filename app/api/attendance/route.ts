...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Failed to fetch attendance", success: false },
      { status: 500 },
    )
  }
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { data, error } = await supabase.from("student_attendance").insert(records).select()
...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Failed to update attendance", success: false },
      { status: 500 },
    )
  }
}
