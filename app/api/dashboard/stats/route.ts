...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Get fee stats
    const { data: feeData } = await supabase.from("student_fees").select("paid, amount")

    const totalFees = feeData?.reduce((sum, f) => sum + (f.amount || 0), 0) || 0
    const paidFees = feeData?.reduce((sum, f) => sum + (f.paid ? f.amount || 0 : 0), 0) || 0
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Failed to fetch stats", success: false },
      { status: 500 },
    )
  }
}
