...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Monthly fees data
    const { data: monthlyFeesData } = await supabase.from("student_fees").select("paid, amount, created_at")
    ...
    const monthlyFees = Array.from({ length: 12 }, (_, i) => {
      ...
      return {
        ...
        collected: monthData?.reduce((sum, f) => sum + (f.paid ? f.amount || 0 : 0), 0) || 0,
      }
    })
    ...
    // Fee status
    const { data: feeData } = await supabase.from("student_fees").select("paid")
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Failed to fetch chart data", success: false },
      { status: 500 },
    )
  }
}
