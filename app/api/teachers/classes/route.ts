  ...
  try {
    ...
    if (!classes || classes.length === 0) {
      try {
        ...
        if (profileErr) {
          console.error("Error fetching profile.class_ids via admin client:", profileErr)
        } else {
        ...
        console.error("Fallback error while fetching classes for teacher:", err)
      }
    }
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Failed to fetch classes", success: false },
      { status: 500 },
    )
  }
}
