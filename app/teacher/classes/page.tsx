...
export default function TeacherClasses() {
  ...
  const loadTeacherData = async (userId: string) => {
    try {
      ...
      const response = await fetch(`/api/teachers/${userId}/classes-with-students`)
      if (!response.ok) {
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="p-4 md:p-8">
        <div className="mb-8">
          ...
          <p className="text-muted-foreground">View your assigned classes and students</p>
        </div>
                ...
                {classes.length === 0 ? (
                  ...
                  classes.map((cls) => {
                    ...
                    return (
                      ...
                      >
                        <span>{cls.name}</span>
                        <span className="text-xs bg-primary/20 px-2 py-1 rounded">{classStudentCount}</span>
                      </Button>
                    )
                  })
                )}
              ...
              <>
                <Card className="p-6 mb-6">
                  <div className="flex items-center justify-between mb-6">
                    <div>
                      <h2 className="text-2xl font-bold text-foreground mb-1">{selectedClass.name}</h2>
                      <p className="text-muted-foreground">Total Students: {filteredStudents.length}</p>
                    </div>
                    ...
                    <table className="w-full">
                      <thead className="bg-secondary border-b border-border">
                        <tr>
                          ...
                          <th className="text-left p-4 font-semibold text-foreground">Father Name</th>
                          <th className="text-left p-4 font-semibold text-foreground">Roll Number</th>
                          ...
                          filteredStudents.map((student) => (
                            <tr key={student.id} className="border-b border-border hover:bg-secondary/50 transition-colors">
                              <td className="p-4 font-medium text-foreground">{student.name}</td>
                               <td className="p-4 font-medium text-foreground">{student.guardian_name}</td>
