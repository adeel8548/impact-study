    ...
    try {
      ...
      await Promise.all(
        data.map(async (teacher: any) => {
          try {
            const res2 = await fetch(`/api/teachers/classes?teacherId=${teacher.id}`)
            const json2 = await res2.json()
            classesData[teacher.id] = json2.classes || []
          } catch (err) {
            console.error("Failed to load classes for teacher", teacher.id, err)
        ...
        <div className="p-4 md:p-8">
          <div className="flex flex-col md:flex-row justify-between items-start mb-8">
            <div>
              ...
              <p className="text-muted-foreground">Manage and assign teachers to classes</p>
            </div>
            <Button onClick={handleAddTeacher} className="gap-2 bg-primary text-primary-foreground">
            ...
            {filteredTeachers.map((teacher) => {
              ...
              return (
                <Card key={teacher.id} className="p-6 hover:shadow-lg transition-shadow">
                  ...
                  {teacher.phone && (
                    <p className="text-xs text-muted-foreground mb-4">{teacher.phone}</p>
                  )}
                  <div className="mt-4">
                    <p className="text-xs font-semibold text-muted-foreground mb-2">Assigned Classes:</p>
