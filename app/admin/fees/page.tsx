...
export default function FeesManagement() {
  ...
  const totalCollected = monthFees.filter((f) => f.paid).reduce((sum, f) => sum + f.amount, 0)
  const totalPending = monthFees.filter((f) => !f.paid).reduce((sum, f) => sum + f.amount, 0)
  ...
  const monthlyCollection = Array.from({ length: 6 }, (_, i) => {
    ...
    return {
      ...
      collected: monthFeeData.filter((f) => f.paid).reduce((sum, f) => sum + f.amount, 0),
      pending: monthFeeData.filter((f) => !f.paid).reduce((sum, f) => sum + f.amount, 0),
    }
  })
      ...
      <div className="md:pl-64">
        <div className="p-4 md:p-8">
          <div className="flex justify-between items-start mb-8">
            <div>
              <h1 className="text-4xl font-bold text-foreground mb-2">Fees Management</h1>
              <p className="text-muted-foreground">Track and manage student fees collection</p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
            <Card className="p-6 border-l-4 border-l-green-500">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-muted-foreground text-sm font-medium mb-1">Collected</p>
                  <p className="text-3xl font-bold text-foreground">₹{totalCollected.toLocaleString()}</p>
                </div>
            ...
            <Card className="p-6 border-l-4 border-l-blue-500">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-muted-foreground text-sm font-medium mb-1">Paid Students</p>
            ...
            <Card className="p-6 border-l-4 border-l-purple-500">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-muted-foreground text-sm font-medium mb-1">Unpaid Students</p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
            {/* Monthly Collection */}
            <Card className="p-6">
              <h3 className="text-lg font-bold text-foreground mb-4">Monthly Collection Trend</h3>
          ...
          <Card className="overflow-hidden">
            <div className="p-6 border-b border-border">
              <h3 className="text-lg font-bold text-foreground">Student Fees Details</h3>
            </div>
              ...
              <table className="w-full">
                ...
                <tbody>
                  {monthFees.slice(0, 8)?.map((fee) => (
                    <tr key={fee.id} className="border-b border-border hover:bg-secondary/50 transition-colors">
                      <td className="p-4 text-foreground font-medium">₹{fee.amount.toLocaleString()}</td>
