...
export default function AttendanceManagement() {
  ...
  const fetchClasses = async () => {
    try {
      ...
      if (Array.isArray(classesData) && classesData.length > 0 && !selectedClass) {
  ...
  const handleStudentAttendanceChange = async (studentId: string, date: string, status: "present" | "absent" | "leave" | null) => {
    try {
      if (!status) {
        // Delete attendance locally for now
        const recordToDelete = studentAttendance.find((a) => a.student_id === studentId && a.date === date)
        if (recordToDelete) {
          setStudentAttendance((prev) => (prev || []).filter((a) => !(a.student_id === studentId && a.date === date)))
        }
        return
      }
      ...
      const existing = (studentAttendance || []).find((a) => a.student_id === studentId && a.date === date)
      ...
      if (!response || !response.ok) throw new Error("Failed to update attendance")
  ...
  const handleTeacherAttendanceChange = async (teacherId: string, date: string, status: "present" | "absent" | "leave" | null) => {
    try {
      if (!status) {
        setTeacherAttendance((prev) => (Array.isArray(prev) ? prev.filter((a) => !(a.teacher_id === teacherId && a.date === date)) : []))
        return
      }
      ...
      const payload = { teacher_id: teacherId, date, status, school_id: user.school_id }
      ...
      <div className="md:pl-64">
        <div className="p-4 md:p-8">
          <div className="mb-8">
            ...
            <p className="text-muted-foreground">Mark and track daily attendance for students and teachers</p>
          </div>
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                      ...
                      <div className="flex flex-col md:flex-row items-center gap-4">
                        <div className="flex  items-center gap-3">
                          ...
                          {(() => {
                            ...
                            const todayRecords = (Array.isArray(studentAttendance) ? studentAttendance : []).filter(
                              (a) => a.date === today,
                            )
                            const presentCount = todayRecords.filter((r) => r.status === "present").length
                            const absentCount = todayRecords.filter((r) => r.status === "absent").length
                            const notMarked = Math.max(0, students.length - (presentCount + absentCount))

                            return (
                              <div className="flex gap-2 items-center">
                                <div className="text-sm">
                                  <div className="text-xs text-muted-foreground">Students — Today</div>
                                  <div className="text-sm font-semibold">P: {presentCount} • A: {absentCount} • N: {notMarked}</div>
                                </div>
                              </div>
                            )
                          })()}
                        ...
                        <div className="flex items-center gap-1 md:gap-2">
                          ...
                          {studentsPastLoaded && (
                            <button
                              ...
                              onClick={() => { setStudentAttendance((prev) => prev.filter((a) => a.date === toLocalDate(new Date()))); setStudentsPastLoaded(false) }}
                            >
                              Clear
                            </button>
                          )}
                  ...
                  {isFetching ? (
                    ...
                    <div className="space-y-4">
                      {students?.map((student) => {
                        const studentRecords = studentAttendance.filter((a) => a.student_id === student.id)
                        return (
                          <div key={student.id} className="border border-border rounded-lg p-4">
                            ...
                            <AttendanceGrid
                              ...
                              onStatusChange={(date, status) =>
                                handleStudentAttendanceChange(student.id, date, status)
                              }
            ...
            <TabsContent value="teachers" className="space-y-6">
              <Card className="p-6">
                <div className="flex flex-col md:flex-row items-center justify-between mb-4">
                  <h3 className="text-lg font-bold text-foreground">Teacher Attendance</h3>

                  <div className="flex flex-col md:flex-row items-center gap-4">
                    {(() => {
                      const today = toLocalDate(new Date())
                      const todayRecords = (Array.isArray(teacherAttendance) ? teacherAttendance : []).filter(
                        (a) => a.date === today,
                      )
                      const presentCount = todayRecords.filter((r) => r.status === "present").length
                      const absentCount = todayRecords.filter((r) => r.status === "absent").length
                      const notMarked = Math.max(0, teachers.length - (presentCount + absentCount))

                      return (
                        <div className="text-sm text-right">
                          <div className="text-xs text-muted-foreground">Teachers — Today</div>
                          <div className="text-sm font-semibold">P: {presentCount} • A: {absentCount} • N: {notMarked}</div>
                        </div>
                      )
                    })()}
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {teachersPastLoaded && (
                        <button
                          className="px-3 py-1 border border-border rounded text-sm"
                          onClick={() => { setTeacherAttendance((prev) => prev.filter((a) => a.date === toLocalDate(new Date()))); setTeachersPastLoaded(false) }}
                        >
                          Clear
                        </button>
                      )}
                ...
                {teachers.length === 0 ? (
                  ...
                  <div className="space-y-4">
                    {teachers?.map((teacher) => {
                      const teacherRecords = (Array.isArray(teacherAttendance) ? teacherAttendance : []).filter((a) => a.teacher_id === teacher.id)
                      return (
                        <div key={teacher.id} className="border border-border rounded-lg p-4">
                          ...
                          <AttendanceGrid
                            ...
                            title=""
                            onStatusChange={(date, status) =>
                              handleTeacherAttendanceChange(teacher.id, date, status)
                            }
